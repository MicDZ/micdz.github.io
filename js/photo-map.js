(function () {
  const mapEls = {
    ...
    middleEast: document.getElementById('photoMapMiddleEast'),
  ...
  if (!mapEls.world || !mapEls.americas || !mapEls.eastAsia || !mapEls.southeastAsia || !mapEls.europe || !mapEls.middleEast ||
  ...
  const bounds = {
    ...
    middleEast: { latMin: 12, latMax: 43, lngMin: 24, lngMax: 64 },
  ...
  const publicConfigElement = document.getElementById('origin-public-config');
  const publicConfig = publicConfigElement ? JSON.parse(publicConfigElement.textContent) : {};
  const githubUser = String((publicConfig.photo_map || {}).github_user || '').trim();
  ...
  const githubLocationCacheKey = `photoMap.githubLocation.v1.${githubUser}`;
  ...
  function updateGithubLocationMarker() {
    if (!githubUser) return Promise.resolve(null);
  ...
  function setActivePane(name) {
    ...
    if (name === 'middleEast' && mapInstances.middleEast) {
      setTimeout(() => mapInstances.middleEast.resize(), paneUpdateDelay);
      setTimeout(() => refreshMapBubbles('middleEast'), paneUpdateDelay);
    }
  ...
  const MIDDLE_EAST = new Set([
    'ARE', 'SAU', 'OMN', 'YEM', 'QAT', 'BHR', 'KWT', 'IRQ', 'IRN',
    'JOR', 'ISR', 'PSE', 'LBN', 'SYR', 'TUR', 'CYP', 'EGY'
  ]);
  ...
  function renderWorldMap(locations) {
    ...
    const meHighlightData = {};
    MIDDLE_EAST.forEach((id) => {
      meHighlightData[id] = { fillKey: 'meHighlight' };
    });
    ...
    map.svg.selectAll('.datamaps-subunit')
      .on('mouseover', function (geo) {
        ...
        } else if (MIDDLE_EAST.has(geo.id)) {
          map.updateChoropleth(meHighlightData);
        }
      })
      .on('mouseout', function () {
      ...
      .on('click', function (geo) {
        ...
        if (AMERICAS.has(geo.id)) {
          ...
          ensureMiddleEastMap(locations);
          zoomToGeoAndSwitch(map, geo, 'middleEast', () => {
            setActivePane('middleEast');
          });
        }
      });
  ...
  function ensureMiddleEastMap(locations) {
    if (mapInstances.middleEast) return;

    const map = new WorldDatamap({
      element: mapEls.middleEast,
      fills: {
        defaultFill: '#d9d9d9',
        pin: '#c12f2f',
        githubPin: '#2ea043'
    ...
    const filterFn = (loc) => {
      return loc.lat >= bounds.middleEast.latMin && loc.lat <= bounds.middleEast.latMax &&
        loc.lng >= bounds.middleEast.lngMin && loc.lng <= bounds.middleEast.lngMax;
    };

    renderMapBubbles('middleEast', map, locations, filterFn, 7);

    mapInstances.middleEast = map;
    setTimeout(() => map.resize(), 0);
  }
